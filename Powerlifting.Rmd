...
PL_Sample <- PL[sample(nrow(PL), 20), ] %>%
  drop_na(MeetTown) %>%
  select(MeetTown)
...
for(i in 1:nrow(PL_Sample))
{
  result <- ggmap::geocode(PL_Sample$MeetTown[i], output = "latlona", source = "google")
  PL_Sample$lon[i] <- as.numeric(result[1])
  PL_Sample$lat[i] <- as.numeric(result[2])
...
ggplot() +
  geom_sf(data = PL_Sample, colour = "red")
