...
PL_Sample <- PL[sample(nrow(PL), 10), ] 

nrow <- nrow(PL_Sample)
counter <- 1
PL_Sample$lon[counter] <- 0
PL_Sample$lat[counter] <- 0
while (counter <= nrow){
  CityName <- gsub(' ','%20',PL_Sample$MeetTown[counter]) #remove space for URLs
  CountryCode <- PL_Sample$MeetCountry[counter]
  url <- paste(
  ...
  x <- fromJSON(url)
  if(is.vector(x)){
    PL_Sample$lon[counter] <- x[[1]]$lon
    PL_Sample$lat[counter] <- x[[1]]$lat    
  }
  counter <- counter + 1
}
...
PL$lat[counter] <- 0
while (counter <= nrow){
  CityName <- gsub(' ','%20',PL$MeetTown[counter]) #remove space for URLs
  CountryCode <- PL$MeetCountry[counter]
  url <- paste(
    "http://nominatim.openstreetmap.org/search?city="
    , CityName
    , "&countrycodes="
