...
```{r explore, cache=TRUE, message=FALSE, warning=FALSE, include=TRUE}
# data summary
...
for(i in 1:nrow(PL))
{
  result <- ggmap::geocode(PL$MeetTown[i], output = "latlona", source = "google")
  PL$lon[i] <- as.numeric(result[1])
  PL$lat[i] <- as.numeric(result[2])
  PL$geoAddress[i] <- as.character(result[3])
}

PL_test <- PL %>%
  filter(lat < 49.3457868,
         lat > 24.7433195,
         lon < -66.9513812,
         lon > -124.7844079) %>%
